...
!% The 'IPModel_EAM_table' object contains all the parameters (the grid points) 
!% read from an 'EAM_table_params' XML stanza.
!X
!XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
!XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
#include "error.inc"

module IPModel_EAM_table_module
...
public :: IPModel_EAM_table
type IPModel_EAM_table
  integer :: n_types = 0
...
end type IPModel_EAM_table

logical, private :: parse_in_ip, parse_matched_label
type(IPModel_EAM_table), private, pointer :: parse_ip
...
interface Initialise
  module procedure IPModel_EAM_table_Initialise_str
end interface Initialise

interface Finalise
  module procedure IPModel_EAM_table_Finalise
end interface Finalise

interface Print
  module procedure IPModel_EAM_table_Print
end interface Print

interface Calc
  module procedure IPModel_EAM_table_Calc
end interface Calc

contains

subroutine IPModel_EAM_table_Initialise_str(this, args_str, param_str)
  type(IPModel_EAM_table), intent(inout) :: this
  ...
  if (.not. param_read_line(params, args_str, ignore_unknown=.true.,task='IPModel_EAM_table_Initialise_str args_str')) then
    call system_abort("IPModel_EAM_table_Initialise_str failed to parse label from args_str="//trim(args_str))
  endif
  call finalise(params)

  call IPModel_EAM_table_read_params_xml(this, param_str)

end subroutine IPModel_EAM_table_Initialise_str

subroutine IPModel_EAM_table_Finalise(this)
  type(IPModel_EAM_table), intent(inout) :: this
  ...
  this%n_types = 0
  this%label = ''
end subroutine IPModel_EAM_table_Finalise
...
subroutine IPModel_EAM_table_Calc(this, at, e, local_e, f, virial, local_virial, args_str, mpi, error)
  type(IPModel_EAM_table), intent(inout) :: this
  ...
  if (present(local_e)) then
     call check_size('Local_E',local_e,(/at%N/),'IPModel_EAM_table_Calc', error)
     local_e = 0.0_dp
  endif

  if (present(f)) then 
     call check_size('Force',f,(/3,at%N/),'IPModel_EAM_table_Calc', error)
     f = 0.0_dp
  end if
  ...
  if (present(local_virial)) then
     call check_size('Local_virial',local_virial,(/9,at%N/),'IPModel_EAM_table_Calc', error)
     local_virial = 0.0_dp
  endif
  ...
  if(present(args_str)) then
     ...
     if (.not. param_read_line(params,args_str,ignore_unknown=.true.,task='IPModel_EAM_table_Calc args_str')) &
     call system_abort("IPModel_EAM_table_Calc failed to parse args_str='"//trim(args_str)//"'")
     call finalise(params)

     if( has_atom_mask_name ) then
        if (.not. assign_pointer(at, trim(atom_mask_name) , atom_mask_pointer)) &
        call system_abort("IPModel_EAM_table_Calc did not find "//trim(atom_mask_name)//" propery in the atoms object.")
     else
        atom_mask_pointer => null()
     endif
  else
...
end subroutine IPModel_EAM_table_Calc

function eam_spline_V(this, ti, tj, r)
  type(IPModel_EAM_table), intent(in) :: this
...
function eam_spline_rho(this, ti, tj, r)
  type(IPModel_EAM_table), intent(in) :: this
  integer, intent(in) :: ti, tj
  real(dp), intent(in) :: r
  real(dp) :: eam_spline_rho

  if (r < min_knot(this%rho(ti,tj)) .or. r >= max_knot(this%rho(ti,tj))) then
    eam_spline_rho = 0.0_dp
  else
    eam_spline_rho = spline_value(this%rho(ti,tj),r)
  endif
...
function eam_spline_rho_d(this, ti, tj, r)
  type(IPModel_EAM_table), intent(in) :: this
  integer, intent(in) :: ti, tj
  real(dp), intent(in) :: r
  ...
  if (r < min_knot(this%rho(ti,tj)) .or. r >= max_knot(this%rho(ti,tj))) then
    ...
    eam_spline_rho_d = spline_deriv(this%rho(ti,tj),r)
  endif
...
subroutine IPModel_startElement_handler(URI, localname, name, attributes)
  ...
  if (name == 'EAM_table_params') then ! new Ercolessi Adams stanza

    if (parse_in_ip) &
      call system_abort("IPModel_startElement_handler entered EAM_table_params with parse_in true. Probably a bug in FoX (4.0.1, e.g.)")
	...
	call finalise(parse_ip)
      ...
      if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find n_types")
      ...
      if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find n_spline_V")
      read (value, *) n_spline_V
      call QUIP_FoX_get_value(attributes, "n_spline_rho", value, status)
      if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find n_spline_rho")
      read (value, *) n_spline_rho
      call QUIP_FoX_get_value(attributes, "n_spline_F", value, status)
      if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find n_spline_F")
      read (value, *) n_spline_F

      allocate(parse_ip%spline_V(5,n_spline_V,n_types,n_types))
      allocate(parse_ip%spline_rho(5,n_spline_rho,n_types,n_types))
  ...
  elseif (parse_in_ip .and. name == 'per_type_data') then
    ...
    if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find type")
    read (value, *) ti

    if (ti < 1 .or. ti > parse_ip%n_types) call system_abort("IPModel_EAM_table_read_params_xml got" // &
    ...
    if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find atomic_num")
  ...
  elseif (parse_in_ip .and. name == 'per_pair_data') then
    ...
    if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find atomic_num_i")
    read (value, *) Zi
    call QUIP_FoX_get_value(attributes, "atomic_num_j", value, status)
    if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find atomic_num_j")
    read (value, *) Zj
    ...
    if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find r_min")
    read (value, *) parse_ip%r_min(parse_cur_type_i, parse_cur_type_j)

    call QUIP_FoX_get_value(attributes, "r_cut", value, status)
    if (status /= 0) call system_abort ("IPModel_EAM_table_read_params_xml cannot find r_cut")
  ...
  elseif (parse_in_ip .and. name == 'point') then
    ...
    if (parse_in_spline_V) then
      ...
      if (parse_cur_point > size(parse_ip%spline_V,2)) call system_abort ("IPModel_EAM_table got too " // &
	...
	parse_ip%spline_V(1:5,parse_cur_point,parse_cur_type_j,parse_cur_type_i) = &
      ...
      if (parse_cur_point > size(parse_ip%spline_rho,2)) call system_abort ("IPModel_EAM_table got " // &
	"too many points " // parse_cur_point // " type " // parse_cur_type_i // " in_spline_rho")
      ...
      if (parse_cur_point > size(parse_ip%spline_F,2)) call system_abort ("IPModel_EAM_table got " // &
...
subroutine IPModel_EAM_table_read_params_xml(this, param_str)
  type(IPModel_EAM_table), intent(inout), target :: this
  ...
  if (this%n_types == 0) call system_abort("EAM_table Tried to parse, but n_types still 0")

  do ti=1, this%n_types
    call initialise(this%F(ti),this%spline_F(1,:,ti),this%spline_F(2,:,ti),huge(1.0_dp),huge(1.0_dp))
    do tj=1, this%n_types
      call initialise(this%rho(ti,tj),this%spline_rho(1,:,ti,tj),this%spline_rho(2,:,ti,tj),huge(1.0_dp),0.0_dp)
...
end subroutine IPModel_EAM_table_read_params_xml
...
subroutine IPModel_EAM_table_Print (this, file)
  type(IPModel_EAM_table), intent(in) :: this
  type(Inoutput), intent(inout),optional :: file

  integer :: ti, tj

  call Print("IPModel_EAM_table : n_types = " // this%n_types // " cutoff = " // this%cutoff, file=file)

  do ti=1, this%n_types
    call Print("IPModel_EAM_table : type " // ti // " atomic_num " // this%atomic_num(ti), file=file)
    call verbosity_push_decrement()
    call Print("IPModel_EAM_table : spline_F ", file=file)
    call Print(this%F(ti), file=file)
    do tj=1, this%n_types
      call Print("IPModel_EAM_table : pair "// ti // " " // tj // " r_min " // this%r_min(ti,tj) // " r_cut " // this%r_cut(ti,tj), file=file)
      call Print("IPModel_EAM_table : spline_rho ", file=file)
      call Print(this%rho(ti,tj), file=file)
      call Print("IPModel_EAM_table : pair spline_V ", file=file)
      call Print(this%V(ti,tj), file=file)
    end do
    call verbosity_pop()
  end do
    
end subroutine IPModel_EAM_table_Print

end module IPModel_EAM_table_module
