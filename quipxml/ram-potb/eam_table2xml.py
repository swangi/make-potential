"""
Convert EAM/fs tabulated functions to QUIP XML format
...
print '<EAM_table_params n_types="%d" n_spline_V="%d" n_spline_rho="%d" n_spline_F="%d">' % (ntypes, nspline, nspline, nspline)
...
for ti, (spi, zi) in enumerate(zip(species, atomic_nums)):
    for tj, (spj, zj) in enumerate(zip(species, atomic_nums)):
        ...
        rho_ij = np.loadtxt('f%s.plt' % sp)
        rmin = rho_ij[:, 0].min()
        rcut = rho_ij[:, 0].max()
        print '<per_pair_data atomic_num_i="%d" atomic_num_j="%d" r_min="%f" r_cut="%f">' % (zi, zj, rmin, rcut)
        print '<spline_rho>'
        for r, rho in rho_ij[::len(rho_ij) / nspline]:
